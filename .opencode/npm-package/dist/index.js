 ...
 */
function isInQuietHours(quietHoursRanges) {
    if (quietHoursRanges.length === 0)
        return false;
    const now = new Date();
    const currentMinutes = now.getHours() * 60 + now.getMinutes();
    for (const range of quietHoursRanges) {
        if (range.startMinutes < 0 || range.endMinutes < 0)
            continue;
...
const COALESCE_CATEGORIES = new Set((process.env.NOTIFY_COALESCE_CATEGORIES || "info,warning")
    .split(",")
    .map((s) => s.trim()));
const RETRY_CATEGORIES = new Set((process.env.NOTIFY_RETRY_CATEGORIES || "critical,blocked")
    .split(",")
    .map((s) => s.trim()));
...
function getBoolEnv(key, fallback) {
...
function buildCategoryConfigs() {
    ...
    for (const category of Object.keys(configs)) {
        ...
        configs[category] = {
            ...
            channel: process.env[channelKey] || configs[category].channel,
            persona: process.env[personaKey] || configs[category].persona,
...
const NOTIFICATION_SPEAK_CATEGORIES = new Set((process.env.NOTIFICATION_SPEAK || "critical,completion,error")
    .split(",")
    .map(s => s.trim()));
// ============================================================================
